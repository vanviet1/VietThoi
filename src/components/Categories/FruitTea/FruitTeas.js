...
const FruitTeas = () => {
  ...
  const fruiteas = products.filter((item) => item.category === "Nước ép");
  ...
  useEffect(() => {
    const sortArray = (type) => {
      if (type === "priceNewAsc") {
        ...
        setData(sorted);
      } else if (type === "productNameAsc") {
        const types = {
          productNameAsc: "productName",
        };
        const sortProperty = types[type];
        const sorted = [...fruiteas].sort((a, b) =>
  ...
  const onChange = (value) => {
    setData(
      [...fruiteas].filter(
        (item) => item.priceNew <= value[1] && value[0] <= item.priceNew
      )
    );
  };
  ...
  const getTaskCurrentPage = () => {
    ...
    return [
      ...dataFilterSearch.slice(startIndex, startIndex + LIMIT_TASK_IN_PAGE),
    ];
  };
  ...
  return (
    <>
      <div className="container">
        <BreadCrumb title="Nước ép" />
      </div>
      <div className="products mb-5">
        <div className="container">
          <h4 className="text-uppercase">Nước ép</h4>
          <>
